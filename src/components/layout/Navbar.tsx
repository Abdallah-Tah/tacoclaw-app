...
import { LayoutDashboard, UserCircle, Activity, Menu, X, Sun, Moon, Bell } from 'lucide-react';
      ...
      <div className="flex items-center gap-4 sm:gap-10">
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          >
            ...
            Notifications
            {unreadCount > 0 && (
              <span className="min-w-[18px] h-[18px] flex items-center justify-center px-1 rounded-full bg-rose-500 text-white text-[9px] font-bold">
        ...
        <div className="absolute top-16 left-0 right-0 glass border-t border-taco-border p-4 md:hidden animate-in slide-in-from-top-4 duration-300 z-[70]">
          <div className="flex flex-col gap-2">
            ...
            <Link
              to="/notifications"
              className={`px-4 py-3 rounded-xl text-base font-semibold flex items-center gap-3 ${
                location.pathname === '/notifications' ? 'bg-orange-500/20 text-orange-100' : 'text-orange-200/60'
              }`}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              <Bell className="w-5 h-5" /> Notifications
              {unreadCount > 0 && (
                <span className="min-w-[20px] h-[20px] flex items-center justify-center px-1 rounded-full bg-rose-500 text-white text-[10px] font-bold">
                  {unreadCount > 99 ? '99+' : unreadCount}
                </span>
              )}
            </Link>
